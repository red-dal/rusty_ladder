...
fn generate_data(size: usize, seed: u64) -> DestinationContainer {
	...
	for _ in 0..size {
		loop {
			...
			if dc.push_domain(SmolStr::from(domain.as_str())) {
				break;
			}
		}
	}
	...
	assert!(!dc.contains(&SocksDestination::new_domain_str("this.is.invalid.domain")));
	dc
}

pub fn criterion_benchmark(c: &mut Criterion) {
	let host = &SocksDestination::new_domain_str("this.is.invalid.domain");
	{
